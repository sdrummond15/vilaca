...
class KinshipsControllerKinships extends JControllerForm
{
...
class PHP_email extends PHPMailer
{
    ...
    public static function email_confirma_contrato($dataForm)
    {
        ...
        $query->select('*');
        $query->from('#__kinships');
        $query->where('id = ' . $dataForm['id']);
        $db->setQuery($query);
        $kinship = $db->loadObjectList();
        ...
        $mail->setBody(
            '<html>'
            . '<body>'
            . '<p>Inscrito no evento: ' . $kinship[0]->name . ' com sucesso.</p>'
        ...
        $mail->setBody(
            '<html>'
            . '<body>'
            . '<p>Inscrição no evento: ' . $kinship[0]->name . ' com sucesso.</p>'
